...
class RNN:
    def __init__(self, note_vector_size, tick_vector_size, velocity_vector_size, hidden_size, unroll_steps):
        ...
        self.vector_size = self.note_vector_size + self.tick_vector_size + self.velocity_vector_size
    ...
    def build_graph(self):
        ...
        for t in range(self.unroll_steps):
            ...
            y_pred_tick = tf.nn.softmax(y_pred[self.note_vector_size:self.note_vector_size+self.tick_vector_size])
            ys_tick.append(y_pred_tick)

            # Predict the next velocity
            y_pred_velocity = tf.nn.softmax(y_pred[self.note_vector_size+self.tick_vector_size:])
        ...
        y_tick = self.y[:, self.note_vector_size:self.note_vector_size+self.tick_vector_size]
        y_velocity = self.y[:, self.note_vector_size+self.tick_vector_size:]
        ...
        with tf.name_scope("accuracy"):
            ...
            y_stacked = tf.stack([tf.argmax(y_note, 1), tf.argmax(y_tick, 1), tf.argmax(y_velocity, 1)], axis=1)
            ys_stacked = tf.stack([tf.argmax(ys_note, 1), tf.argmax(ys_tick, 1), tf.argmax(ys_velocity, 1)], axis=1)
    ...
    def prepare_for_training(self, sess):
        ...
        initial_loss = -np.log(1.0/self.note_vector_size) + -np.log(1.0/self.tick_vector_size) + -np.log(1.0/self.velocity_vector_size)
    ...
    def sample(self, sess, h, c, seed_ix_note, seed_ix_tick, seed_ix_velocity, n):
        ...
        h, c: ndarray of shape (hidden_size, 1)
            the starting memory state
        seed_ix_note/tick/velocity: int
        ...
        for t in range(n):
            ...
            x[0, self.note_vector_size + self.tick_vector_size + seed_ix_velocity] = 1
            ...
            ops = [self.y_pred_note, self.y_pred_tick, self.y_pred_velocity, self.first_h, self.first_c]
            predicted_note, predicted_tick, predicted_velocity, h, c = sess.run(ops, feed_dict=feed)
            ...
            ixes.append((ix_note, ix_tick, ix_velocity))

            # Use the output as the next input.
            x[0, seed_ix_note] = 0
            x[0, self.note_vector_size + seed_ix_tick] = 0
            x[0, self.note_vector_size + self.tick_vector_size + seed_ix_velocity] = 0
...
class Data:
    def __init__(self, filename):
        ...
        self.ix_to_note = pickle.load(open("notes.p", "rb"))
        self.ix_to_tick = pickle.load(open("ticks.p", "rb"))
        self.ix_to_velocity = pickle.load(open("velocities.p", "rb"))

        print self.ix_to_note
        print self.ix_to_tick
    ...
    def to_text(self, ixes):
        ...
        return ",".join(str(self.ix_to_note[ix_note]) + ":" + \
                        str(self.ix_to_tick[ix_tick]) + ":" + \
                        str(self.ix_to_velocity[ix_velocity]) for ix_note, ix_tick, ix_velocity in ixes)

################################################################################

def write_midi_file(filename, notes_ticks_velocities):
    ...
    for note, tick, velocity in notes_ticks_velocities:
        track.append(mido.Message('note_on', note=note, time=tick, velocity=velocity))
    ...
    with tf.Session(config = config) as sess:
        ...
        for n in range(start_n, steps + 1):
            ...
            if n > 0 and n % 1000 == 0:
                ...
                seed_ix_tick = np.argmax(x[0, data.unique_notes:data.unique_notes+data.unique_ticks])
                seed_ix_velocity = np.argmax(x[0, data.unique_notes+data.unique_ticks:])
                sampled = rnn.sample(sess, h, c, seed_ix_note, seed_ix_tick, seed_ix_velocity, 400)
...
def sample(rnn, data):
    ...
    with tf.Session() as sess:
        ...
        first_ix_tick = 0
        first_ix_velocity = 0
        sampled = rnn.sample(sess, h, c, first_ix_note, first_ix_tick, first_ix_velocity, 1000)
        print("----\n%s\n----" % data.to_text(sampled))

        notes = []
        for ix_note, ix_tick, ix_velocity in sampled:
            notes.append((data.ix_to_note[ix_note], data.ix_to_tick[ix_tick], data.ix_to_velocity[ix_velocity]))
...
def random_notes(data):
    ...
    for i in range(200):
        ...
        notes.append((data.ix_to_note[note_ix], data.ix_to_tick[tick_ix], data.ix_to_velocity[velocity_ix]))
...
data = Data("X.npy")
rnn = RNN(data.unique_notes, data.unique_ticks, data.unique_velocities, hidden_size, unroll_steps)
